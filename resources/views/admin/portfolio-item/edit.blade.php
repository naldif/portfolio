            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.portfolio-item.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-sm-12 col-md-7">
                                    <select class="form-control selectric" name="category_id">
                                      ...
                                      @foreach ($category as $item)
                                          <option value="{{ $item->id }}" {{ $item->id == $portfolio->category_id ? 'selected' : '' }}>{{ $item->name }}</option>
                                      @endforeach
                                    </select>
                                  </div>
                            </div>
                            <div class="form-group row mb-4">
                                ...
                                <div class="col-sm-12 col-md-7">
                                    <textarea class="summernote" name="description">{!! $portfolio->description !!}</textarea>
                                </div>
                            </div>
                            <div class="form-group row mb-4">
...
<script>
    $(document).ready(function(){
            $('#image-preview').css({
                'background-image': 'url("{{ asset($portfolio->image) }}")',
